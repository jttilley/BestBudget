    ...
    <Box sx={{
      ...
      <Box sx={{display: 'inline-grid',
        ...
        { category === 'Income' ? <Typography><SavingsIcon fontSize="small" sx={{color: 'green', alignItems: 'flex-end'}}/>${budgetTotal.toFixed(2)}</Typography> :
          <>
            <Typography sx={{color:'#666666', textAlign:'right'}}> <i> Suggested:</i></Typography>
      ...
      { category === 'Income' ? <></> :
        <Box sx={{display: 'inline-grid',
          ...
          <>
            { budgLeft >= 0 ? 
              <Typography sx={{color: 'green', textAlign: 'left'}}>${budgLeft.toFixed(2)}</Typography> 
              :
              <Typography sx={{color: 'red', textAlign: 'left'}}>${budgLeft.toFixed(2)}</Typography>
            }
